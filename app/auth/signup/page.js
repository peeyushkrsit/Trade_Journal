...
import { auth, db, GoogleAuthProvider } from '@/lib/clientFirebase';
  ...
  const handleEmailSignup = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', user.uid), {
        displayName: displayName || user.email?.split('@')[0] || 'Trader',
        email: user.email,
        createdAt: new Date(),
        plan: 'free',
        analysisCount: 0,
        analysisMonth: new Date().toISOString().slice(0, 7), // YYYY-MM
      });
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="bg-card p-8 rounded-lg border border-gray-800">
          ...
          <p className="text-gray-400 text-center mb-8">Start tracking your trades today</p>
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-3 bg-white text-gray-900 rounded-lg font-semibold hover:bg-gray-100 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
          >
        ...
        <div className="mt-6 p-4 bg-card rounded-lg border border-yellow-600/30">
          <p className="text-yellow-400 text-sm font-semibold mb-1">⚠️ Important Disclaimer</p>
